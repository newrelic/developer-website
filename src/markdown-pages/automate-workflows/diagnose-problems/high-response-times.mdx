...
duration: '15 min'
title: 'Diagnosing high response times in Telco Lite'
...
This tutorial assumes you're familiar with the [infrastructure of Telco Lite](diagnose-problems#welcome-to-acme-telco-lite), you've [set up your local environment](diagnose-problems#set-up-your-environment), and you've [deployed and instrumented the Telco Lite services](diagnose-problems#deploy-telco-lite).

<Steps>

<Step>

Log in to [New Relic One](one.newrelic.com) and select **APM** from the top navigation menu. Here, you see an overview of all eight Telco Lite services, including the service names, response times, and throughputs. Notice that the response time for **Telco-Warehouse Portal** is unusually high:
...
External web traffic is the primary contributor to the high response times. That's a good start, but "external web traffic" is vague and unhelpful. Next, you dive deeper into the specifics of what makes up that component.
...
This trace shows you that one external request contributes almost all of the total trace duration. Specifically, an external request to the **Telco-Fulfillment Service** contributes over 99% of the overall response time.
...
Now, you're looking at graphs in the infrastructure view for that service's host. Notice that **CPU %** has a lot of high spikes. Click and drag on the graph from the start of a spike to the end of it to narrow the time slice to the period when CPU utilization goes up:
...
Notice that the web portal's response times aren't nearly as high as those in the warehouse portal.
...
You can learn more about using New Relic by diagnosing [other issues](diagnose-problems#view-your-services). If this is your last issue, you can [tear down](diagnose-problems#tear-down-telco-lite) all the Telco Lite services.