...
In this code block, you're setting the required version of Terraform to 0.13.x and setting the New Relic provider to the latest 2.x version.

We recommend pinning your New Relic provider version by setting a [version constraint](https://www.terraform.io/docs/configuration/version-constraints.html) similar to the example (`~> 2.12`). If you only want to update with patch releases, use `~> 2.12.0`. Using the right version constraint for your setup can provide better stability with your Terraform runs.
...
While it's running, Terraform will send logs to your console:
...
To start off, you can create an email notification channel to send alert notifications to one or more emails. This can be useful when you want to notify a specific person or team when alerts are triggered.
...
This step is optional, but since Slack is quite popular, creating a Slack notification channel can be a useful integration. When an alert triggers, a notification will be sent to the Slack channel of your choice.

<Important>

Before adding a Slack notification channel, you need to add the [New Relic Slack App](https://slack.com/apps/A0F827KK2-new-relic-alerts) to your Slack account and select a Slack channel to send the notification.
