...
Now that you have a high-level understanding of the tools you'll use, you can begin by building the Demo Deployer!
...
To use the deployer, first clone its repository from GitHub. Then, build a Docker image using the local copy of the code.
...
Now, you have a Docker image, named `deployer`, which you use throughout the rest of this guide.
...
docker run -it\
    ...
    --entrypoint ruby deployer main.rb -c configs/creds.json -d demo-deployer/blob/main/demo/catalog/hello.aws.json
```
...
- `-d demo-deployer/blob/main/demo/catalog/hello.aws.json`: The [deployment configuration file](https://github.com/newrelic/demo-deployer/tree/main/documentation/deploy_config)
...
The deployment configuration file, [hello.aws.json](https://github.com/newrelic/demo-deployer/blob/main/demo/catalog/hello.aws.json), which drives the deployer, contains four sections:
...
docker run -it\
    -v $HOME/configs/:/mnt/deployer/configs/\
    --entrypoint ruby deployer main.rb -c configs/creds.json -d demo-deployer/blob/main/demo/catalog/hello.aws.json -t
```
