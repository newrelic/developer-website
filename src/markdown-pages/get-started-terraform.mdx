...
duration: '20 min'
title: 'Use New Relic with Terraform'
template: 'GuideTemplate'
description: "Learn how to set up New Relic with [Terraform](https://www.terraform.io/) and create alerts with notifications."
---
...
To follow this guide, you should have some basic knowledge of both New Relic and Terraform.
...
To connect Terraform with New Relic, you must first set New Relic as a provider. This is necessary so that Terraform can create and manage New Relic resources in your account.
...
`plan` is a [dry run](https://en.wikipedia.org/wiki/Dry_run_(testing)), so nothing is actually provisioned. Always run `plan` to test your configuration before applying it.
...
First, to store your application's information, you must add a [data source](https://www.terraform.io/docs/configuration-0-11/data-sources.html):
...
Now you can add the alert condition:
...
The `critical` alert condition you've just defined is triggered when the [Apdex score](https://docs.newrelic.com/docs/apm/new-relic-apm/apdex/apdex-measure-user-satisfaction) of your application falls below `0.75` for five minutes. Feel free to modify the settings.
...
To set up a notification channel to your alert policy, add the following snippet to your configuration file:
...
The last step is provisioning the resources you've configured in `main.tf`. This enables them in your New Relic account for your application.
...
This avoids changes to resources caused by configuration items.
