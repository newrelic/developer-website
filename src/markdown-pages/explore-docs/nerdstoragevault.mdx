...
A trusted nerdpack ID header value is provided by service gateway when requests come from nerdlets and
...
```json copy=false lineNumbers=false
mutation {
  nerdStorageVaultDeleteSecret(scope: { actor: CURRENT_USER } , key: "test_key" ) {
    status
    errors {
      message
    }
