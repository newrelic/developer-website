...
To follow the examples in this gude, the accompanying example code is available on [GitHub](https://github.com/jsbnr/nr-terragrunt-intro-example)
...
```
In the new directory, create a `terragrunt.hcl` file, create a  ./src directory, and create a `./evironments` directory:
```bash
touch terragrunt.hcl
mkdir src
mkdir environments
```
Inside of your __environments__ folder, create a folder named __dev.__
In the `./src` directory, create a __main.tf__ and __provider.tf__ fiiles:
```bash
...
In the `./enviromments/dev` directory create a new file named `terragrunt.hcl`
```bash
cd ./enviroments/dev/
touch terragrunt.hcl
```

Inside the dev folder __terragrunt.hcl__ add an include statement. The include statement tells Terragrunt to use any `.hcl` confgiuration files it finds in parent folders.
```hcl
include {
...
Next, add a Terraform block to provide Terragrunt with a Terraform soruce
```hcl
terraform {
	source = "../../src"
}
...
Now you need to configure the provider with your New Relic account details.
```hcl
provider "newrelic" {
...
Tip: Documentation about the New Relic provider is provided on the [Terraform doc site.](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs)

Update the __provider.tf__ file to include your New Relic account information using variables keeping the Terraform configuration dynamic.
```hcl
...
In your terminal, change directories to the environment/dev folder and run the command `terragrunt init`:
```bash
terragrunt init
```
...
Terragrunt starts to run but pauses to ask for the inputs to the variables added to the __provider.tf__ file.
```bash copyable=false
[output] {muted}# Example output
[output] ------------------------------------------------------------------------
[output]
...
Exit the `terragrunt plan` process. In the environments/dev __terragrunt.hcl__ file, add an inputs block to provide the values for your New Relic account vairables:
```hcl
inputs = {
...
Remove the New Relic account variables added to the provider.tf file in the previous steps. The updated terragrunt.hcl only has the include, terraform, and inputs block
```hcl
include {
	path= find_in_parent_folders()
}
...
Now, when you run `terragrunt plan`, Terragrunt will provide the input block's values, no longer prompting the command line for values.  Run `terragrunt plan`:
...
Run `terragrunt apply`. After Terraform finishes process, check your New Relic account for the new Demo Policy in your New Relic account.
</Step>
...
Now that you have a basic New Relic configuration created, you add configurations built in the [Getting Started with Terraform](/terraform/get-started-terraform) and [Terraform modules](/terraform/terraform-modules) guides in this step.

<Callout variant="tip">
If you haven't done the previous developer guides, you can copy the configurations for this section from the <a href="https://github.com/jsbnr/nr-terragrunt-intro-example">Terragraunt Intro Github Repo</a>.
</Callout>

Update the __main.tf__ to include the addtional terraform configurations. Update the emaiil address in the alert channel to your prefered email address:
```hcl
resource "newrelic_alert_policy" "DemoPolicy" {
	 name = "My Demo Policy"
	}
...
You have added a New Relic alert channel, subscribed the DemoPolicy to the alert channel, and added a module hosted on Github.

To clone the module from Github, run `terragrun init` and then run `terragrunt apply`. After Terraform is done processing, You will see your Demo Policy now has two conditions in your New Relic account and one alert channel attached.
...
Using Terragrant, you can add the name of the environment you are running to the name of the data you are creating, making your resource more identifiable in New Relic.
...
in the __main.tf__ file add a new varialble for 'env_name'
```hcl
variable "env_name" {}
```

Using string interpolation add the new __env_name__ variable to the alert policy and alert channel resource blocks.
```hcl
resource "newrelic_alert_policy" "DemoPolicy" {
...
Currently, the environment variable name is hardcoded into the root __terragrunt.hcl__ file. You want to avoid having your variables hardcode, using a terragrunt built-in function will pass the value.
...
Run `terrafgrunt plan`. Looking that the output, the env_name variable will have the entire `./environments/dev/` path.
```bash copyable=false
[output] {muted}# Example output
[output] ------------------------------------------------------------------------
[output]
...
Run `terrafgrunt plan`. Looking that the output the env_name variable will now have 'dev:' added.   Run `terragrunt apply` to update your configurations.
```bash copyable=false
[output] {muted}# Example output
[output] ------------------------------------------------------------------------
[output]
...
Now, update the remote_state block to provide your S3 bucket and update the key to use the local enviroment variable created previously.
```hcl
 remote_state {
  backend = "s3"
  generate = {
...
<Callout variant="tip">
To see the files created by Terragrunt, look inside of the ./enviroments/dev directory. There is a hidden directory named __terragrunt-cache.__ Within the __terragrunt-cachev__ directory, you will see the __backend.tf__ file that Terragrunt generated.
</Callout>
...
Now that you have a working configuration for your dev environment, the next steps you will create a new environment resuing the dev configurations.
...
Next, you want to pass the values into your environment configurations by adding the inputs block variables. In the __nonprod terrgrunt.hcl__ fiile, add the cpu_critical, cpu_warning, and diskPercentage variables:
```hcl
inputs = {
	newrelic_personal_apikey = "NRAK-***" # Your New Relic account ID
	newrelic_account_id = "12345" # Your New Relic account ID
...
Congratulations! You've used Terragrunt to generate your New Relic configurations and managed multiple environments.  Review the [New Relic Terraform provider documentation](https://registry.terraform.io/providers/newrelic/newrelic/latest/docs) and [Terragrunt quick start](https://terragrunt.gruntwork.io/docs/getting-started/quick-start/) to learn how you can take your configuration to the next level.
