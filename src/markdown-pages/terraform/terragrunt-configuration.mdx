...
In _main.tf_, update the email address in the alert channel to your preferred email address:
...
You've added a New Relic alert channel, subscribed the demo policy to the alert channel, and added a module hosted on Github.
...
After Terraform finishes processing, your alert policy has two conditions in your and one alert channel.
...
In the _main.tf_, file add a new variable called `env_name`:
...
Adding a `locals` block creates a local variable. Using the replace built-in function removes the unwanted parts of the relative path. And updating the `inputs` block to use the local variable.
