---
path: '/explore-docs/custom-viz/build-visualization'
duration: '15 min'
...
In a browser, open [https://one.newrelic.com/?nerdpacks=local](https://one.newrelic.com/?nerdpacks=local). This url is also shown in the terminal when the Node server starts:

```sh
[output] {success}✔{normal}  Server ready! Test it at: https://one.newrelic.com/?nerdpacks=local
[output] {blue}↩{normal}  Server will reload automatically if you modify any file!
```

The `nerdpacks=local` query string directs the UI to load your visualization from the local server.
...
In **Custom Visualizations**, find and click the tile for your visualization:

![Navigate to your visualization](../../images/custom-viz/nav-to-your-viz.png)

Notice the **</\> Local** label on the tile. This identifies the visualization as locally served and not one that is published to the New Relic One catalog. If you don't see this label or the tile isn't there at all, you may need to restart your server and refresh your browser.

When you select your local visualization, it's rendered with a list of configuration options established in _nr1.json_:

![Default visualization](../../images/custom-viz/default-viz.png)

</Step>

<Step>

Update the **Configuration visualization properties**, and watch your visualization update automatically:
...
Configuring these properties is one way to update your visualization at runtime. Changing your React code is another.
