...
1. In a text editor, open `/add-time-picker/nerdlets/nr1-howto-add-time-picker-nerdlet/index.js`.

2. Add the `PlatformStateContext` component to the end of the import statement so it looks like this:
...
3. Just below the current `return` insert this code for the `PlatformStateContext` component:
...
4. Move the current application code so it is under the `return` of the `PlatformState` function call. The `return` statement should now look like this:
...
5. Add a `console.log` statement to make sure you are seeing appropriate data. Insert the following code inside the `PlatformState` return statement just before the opening tag for the `<Grid>` component:
...
1. Import the `timeRangeToNrql` method by inserting this line of code below the other `import` sections:
...
2. Pass the `PlatformState` to the `timeRangeToNrql` helper, and save its output as a `since` statement for later use.

```js
const since = timeRangeToNrql(PlatformState);
```

3. After creating the `since` variable, go through the code in the `PlatformStateContext` return statement and concatenate the `since` variable to each of the existing chart component queries. Here's a `TableChart` example:

```js
<TableChart fullWidth accountId={this.accountId} query={trxOverview+since} />
```

4. After you update all of the chart components, confirm that the final `index.js` file looks similar to this:
