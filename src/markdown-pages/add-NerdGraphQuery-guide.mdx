...
![Use Nerdgraph launcher](../images/use-nerdgraph-launcher.png)
...
![Screenshot showing the sample transaction application](../images/no-name.png)

## Add the `NerdGraphQuery` component

Now you can add the `NerdGraphQuery` component to build a dropdown menu to change the account the application is viewing.
...
![BROWSER CONSOLE IMAGE](../images/console-screenshot.png)
...
![App Image](../images/with-name.png)
...
Now, you need to make it possible to query NerdGraph outside of the `render` method and save the returned data for later usage throughout the New Relic One application. To do this, use the `NerdGraphQuery.query` method within the `componentDidMount` react lifecycle method.
...
* The `NerdGraphQuery.query` method calls with the query object to get your account data will be stored in a variable called `accounts`. 
* Once the response comes back from NerdGraph, the results are logged to the browser console for your viewing, processed, and stored into the application state.
...
![App Complete](../images/complete.png)
