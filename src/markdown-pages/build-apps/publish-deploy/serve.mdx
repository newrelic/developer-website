...
To load a Nerdpack into the platform, whether it's locally served or published to the Instant Observability catalog, New Relic uses an `<IFRAME>` and a separate domain (a subdomain of nr-ext.net), relying on same-domain policy to sandbox the code.
