...
You've passed New Relic data to _Newsletter subscriptions per version_, but the rest of the charts still use mocked data:
...
Some of these charts use unsubscription data, which you'll learn how to get in a later tutorial, but your demo application creates a custom event for every page view like it does for subscriptions.

Remove existing mock data, and use `NrqlQuery` components to supply page view event data to _Version A - Page views_ and _Version B - Page views_:
...
Well done! Your line charts now show real subscription and page view data from New Relic's database. Note that there are still a few charts that use only page view and subscription data:

- _Total subscriptions per version_
- _Version A - Page views vs. subscriptions_
- _Version B - Page views vs. subscriptions_

These charts have a couple things in common:

- They compare all-time totals between two sets of data
- They are not timeseries charts, but rather show numerical values

Because these charts don't show timeseries and compare values, in some cases across multiple custom events, you have to handle them differently than you did for the line charts. In the next lesson, you'll learn how to supply data to these three charts.