...
Then update the helper with this code beginning `_addToNerdStorage`:
...
Once you get data storage working as described in the section above, you also need to get the app properly reading data from NerdStorage, or the app will reload with an empty state every time you navigate away from the app page and back. To do this, add the `UserStorageQuery` component and update the `1componentDidMount` method. 
