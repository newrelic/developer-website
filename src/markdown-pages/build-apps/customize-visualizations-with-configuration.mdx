...
You used a `SegmentedControl` to switch between the two chart types in the visualization UI. This implementation takes up space in the visualization, but it offers your users the choice to switch between two chart types on the fly. But what if you only need to be able to select an option once, when initializing the visualization?
...
![Configured visualization](../../images/custom-viz/radar-or-treemap-configured.png)
...
This lesson is part of a course that teaches you how to build a custom visualization in the New Relic One platform. When you're ready, continue on to the next lesson: [_Add custom visualizations to your dashboards_](/build-apps/add-visualizations-to-dashboard).
