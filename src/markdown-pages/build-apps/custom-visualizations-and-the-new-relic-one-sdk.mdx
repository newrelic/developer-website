...
In this lesson, you build a visualization that displays your data in one of two chart types: [`RadarChart`](https://recharts.org/en-US/api/RadarChart) or [`Treemap`](https://recharts.org/en-US/api/Treemap). You then implement a `SegmentedControl` component from the New Relic app SDK, which allows you to alternate between the two chart types. Ultimately, this gives you freedom to view your data in a dynamic way that isn't possible with New Relic's base offerings.
...
Ensure you're working with the latest version of the New Relic app CLI:
...
Navigate to _alternate-viz/visualizations/radar-or-treemap/index.js_. You'll work in here for the rest of this lesson.
...
To learn more about the components available in the New Relic app SDK, go to our [Intro to New Relic app SDK](/explore-docs/intro-to-sdk).
...
- Customize your visualization using New Relic app SDK components
...
This lesson is part of a course that teaches you how to build a custom visualization on the New Relic platform. When you're ready, continue on to the next lesson: [_Customize visualizations with configuration_](/build-apps/customize-visualizations-with-configuration).
