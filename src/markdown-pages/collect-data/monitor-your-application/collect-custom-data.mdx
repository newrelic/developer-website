...
Here, you record custom attributes to gather details about your food order.

<Steps>

<Step>

In _server/index.js_ file, update the code to add custom attribute on your server side.

<>

``` node lineHighlight=1,61-73 fileName=server/index.js
...
Here, you add customer name, restaurant name, item count and order total as custom attributes by calling `newrelic.addCustomAttribute` API. 
These custom attributes adds metadata to your transaction event and you record them as a part of your transaction event so there is no overhead. 

</Step>

<Step>

In _app/js/services/cart.js_ file, update the code to add PageAction event.
...
Here, you added a PageAction that records restuarant name, item and item quantity whenever a customer places an order. 
