...
To continue this guide, the important bit is the value of `guid`, which uniquely identifies your application; copy it or write it down.
...
Now that you've the GUID, you can point the New Relic CLI directly to your application. Adding a tag is the simplest way to try out the CLI capabilities (don't worry, tags can be deleted by using [`entity tags delete`](https://github.com/newrelic/newrelic-cli/blob/master/docs/cli/newrelic_entity_tags_delete.md)).
...
You've created and added some tags to your application, but how do you know they're there? You need to retrieve your application's tags for that.
...
To create a deployment marker, run the [`apm deployment create`](https://github.com/newrelic/newrelic-cli/blob/master/docs/cli/newrelic_apm_deployment_create.md) command passing the same Application ID from your earlier search.
