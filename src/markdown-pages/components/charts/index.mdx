...
description: 'How to use charts in your NR1 app.'
resources:
...
With your chart component, set the `accountId` and `query` props to query your New Relic data using [NRQL](<[NRQL](https://docs.newrelic.com/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language)>):
...
This `LineChart` plots a line between coordinates `(0, 0)` and `(20, 10)`. Use x-y coordinates for all two-dimensional chart formats. For other formats, which you'll learn more about [later](#data), use other types of data points.
...
<!-- Author's note: The line breaks and lack of indentation here are intentional—the page doesn't render properly, otherwise. This will change when the site moves to MDX v2, so keep an eye out! -->
